...
namespace TaskManager.DeveloperEvaluation.Common.HealthChecks;
...
public static class HealthChecksExtension
{
    ...
    {
        var livenessOptions = WriteHealtCheckResponse(app, "liveness");
        app.UseHealthChecks("/health/live", livenessOptions);

        var readinessOptions = WriteHealtCheckResponse(app, "readiness");
        app.UseHealthChecks("/health/ready", readinessOptions);

        var healthOptions = WriteHealtCheckResponse(app, string.Empty);
    ...
    /// </remarks>
    private static HealthCheckOptions WriteHealtCheckResponse(this WebApplication app, string tag)
    {
